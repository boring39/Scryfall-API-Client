...
internal class Cards : ICards
{
    ...
    public Task<ResultList<Card>> GetPage(int page, CancellationToken cancellationToken = default)
        => _restService.GetAsync<ResultList<Card>>(new Uri($"{CardsEndPoint}?page={page}",UriKind.Relative), cancellationToken: cancellationToken);

    public Task<Card> GetRandom(CancellationToken cancellationToken = default)
        => _restService.GetAsync<Card>(new Uri($"{CardsEndPoint}/random",UriKind.Relative), false, cancellationToken);
    ...
    public Task<ResultList<Card>> Search(string query, int page, SearchOptions options = default, CancellationToken cancellationToken = default)
    {
        ...
        return _restService.GetAsync<ResultList<Card>>(new Uri($"{CardsEndPoint}/search?q={query}&page={page}&{options.BuildQueryString()}",UriKind.Relative), cancellationToken: cancellationToken);
    }
    ...
    public Task<Card> GetByMultiverseId(int id, CancellationToken cancellationToken = default)
        => _restService.GetAsync<Card>(new Uri($"{CardsEndPoint}/multiverse/{id}", UriKind.Relative), cancellationToken: cancellationToken);
    public Task<Card> GetByTcgPlayerId(int id, CancellationToken cancellationToken = default)
        => _restService.GetAsync<Card>(new Uri($"{CardsEndPoint}/tcgplayer/{id}", UriKind.Relative), cancellationToken: cancellationToken);
    public Task<Card> GetByMtgoId(int id, CancellationToken cancellationToken = default)
        => _restService.GetAsync<Card>(new Uri($"{CardsEndPoint}/mtgo/{id}", UriKind.Relative), cancellationToken: cancellationToken);
    public Task<Card> GetByArenaId(int id, CancellationToken cancellationToken = default)
        => _restService.GetAsync<Card>(new Uri($"{CardsEndPoint}/arena/{id}", UriKind.Relative), cancellationToken: cancellationToken);
    public Task<Card> GetByCardmarketId(int id, CancellationToken cancellationToken = default)
        => _restService.GetAsync<Card>(new Uri($"{CardsEndPoint}/cardmarket/{id}", UriKind.Relative), cancellationToken: cancellationToken);
    public Task<Card> GetByScryfallId(Guid id, CancellationToken cancellationToken = default)
        => _restService.GetAsync<Card>(new Uri($"{CardsEndPoint}/{id}", UriKind.Relative), cancellationToken: cancellationToken);
}
