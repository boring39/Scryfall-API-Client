...
public class BulkData : IBulkData
{
    ...
    {
        ResultList<BulkDataItem> bulkData = await Get();
        return [.. bulkData.Data.Where(d => d.LastUpdated >= updatedSince)];
    }

    public async Task<BulkDataItem> Get(DateTimeOffset updatedSince, string bulkDataType) {
        ICollection<BulkDataItem> a = await Get(updatedSince);
        return a.FirstOrDefault(d => d.Type.Equals(bulkDataType, StringComparison.OrdinalIgnoreCase));
    }
